...
var BRAND = {
  ...
  'LAADV AM': {
    ...
    headerBg:        '#1B3A2D',   // verde escuro
    headerText:      '#C9A84C',   // dourado
    accentColor:     '#C9A84C',
    sectionColor:    '#1B3A2D',
    lightBg:         '#F8F4E8',
...
function createBrandedDoc_(id, payload, now) {
  ...
  // ── 1. FAIXA DE CABEÇALHO (tabela com cor de fundo) ──────
  ...
  // Não há setBorderBottom nativo — usamos uma linha colorida via tabela fina
  ...
  for (var i = 0; i < lines.length; i++) {
    ...
    // Títulos de seção: "1. RESULTADOS X META"
...
function styleMetaTable_(table, brand) {
  ...
  for (var r = 0; r < table.getNumRows(); r++) {
    ...
    var isLabel = (r % 2 === 0); // linhas 0 e 2 são rótulos
